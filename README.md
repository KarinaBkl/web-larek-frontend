...
Класс Model используется для создания моделей данных, которые могут быть связаны с событиями и изменять свои свойства через механизм событий.

Поля класса: 

- `data` - объект частичных данных модели
- `events` - экземпляр интерфейса IEvents

Методы класса: 

- `constructor` - конструктор класса, принимающий data и events
- `emitChanges` - метод для отправки событий в систему событий
...
Класс Form наследуется от Component,  он реализует функциональность формы и отображает данные на странице.
В конструкторе класса создаются ссылки на элементы формы, такие как кнопка отправки и область для отображения ошибок. Добавлены обработчики событий для отслеживания изменений в полях формы и отправки формы.

Поля класса:

- `_submitButton` - ссылка на кнопку отправки формы
- `_contentError` - ссылка на элемент, где будут отображаться ошибки

Методы класса:

- `onInputChange` - вызывается при изменении значения в поле формы
- `setValid` - устанавливает состояние валидации формы
- `setErrors` - устанавливает текст ошибок для отображения
...
- `constructor(container: HTMLElement, events: IEvents)` - конструктор наследуется от абстрактного класса ModalInformation
...
Предназначен для реализации модального окна с формой, содержащей поля ввода email и телефона. При сабмите инициирует событие подтверждения данных и переход на окно с уведомлением об успешном оформлении заказа. Наследуется от класса Form.
