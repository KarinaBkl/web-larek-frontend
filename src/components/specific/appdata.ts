import {ICards, ICammonInfo, IFormError, IAppInfo} from '../../types';
...
export class AppState extends Model<IAppInfo> {
	...
	preview: string | null;
	setProductList(items: ICards[]) {
		this.catalog = items;
		this.emitChanges('items:changed', { catalog: this.catalog });
	}
	addToBasket(item: ICards): void {
		this.basket.push(item);
		this.emitChanges('basket:changed', this.basket);
		
	}
	deleteFromBasket(item: ICards) {
		this.basket = this.basket.filter((basketItem) => basketItem.id !== item.id);
		this.emitChanges('basket:changed', this.basket);
	}
	isInBasket(item: ICards) {
	...
	setField(field: keyof ICammonInfo, value: string) {
